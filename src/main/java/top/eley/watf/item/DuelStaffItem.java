...
public class DuelStaffItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (targetEntity == null) {
            player.displayClientMessage(Component.literal("§c没有看到可以决斗的生物!"), true);
        ...
        if (targetEntity instanceof AgeableMob) {
            player.displayClientMessage(Component.literal("§c这个生物太温顺了，不会战斗!"), true);
            return InteractionResultHolder.fail(stack);
        }

        if (targetEntity.equals(player)) {
            player.displayClientMessage(Component.literal("§c不能让自己打架!"), true);
            return InteractionResultHolder.fail(stack);
        }
        ...
        if (!FIRST_TARGET.containsKey(playerId)) {
            ...
            player.displayClientMessage(Component.literal("§a已选择第一个选手: §e" + name + "§a，再选一个!"), true);
            ...
            if (firstEntity.equals(targetEntity)) {
                player.displayClientMessage(Component.literal("§c选个不同的对手!"), true);
                return InteractionResultHolder.fail(stack);
            }

            if (!firstEntity.isAlive() || firstEntity.isRemoved()) {
                player.displayClientMessage(Component.literal("§c第一个选手已经没了!"), true);
                return InteractionResultHolder.fail(stack);
            }

            if (!(firstEntity instanceof LivingEntity firstLiving)) {
                player.displayClientMessage(Component.literal("§c第一个选手已经不能战斗了!"), true);
                return InteractionResultHolder.fail(stack);
            }
            ...
            player.displayClientMessage(Component.literal("§6§l决斗开始! §e" + name1 + " §cVS §e" + name2), true);
